...
async function llamarGemini(prompt) {
  ...
  const body = {
    contents: [{ parts: [{ text: prompt + " Por favor responde en español." }] }]
  };
...
async function llamarCohere(prompt) {
  ...
  const body = {
    ...
    prompt: prompt + " Por favor responde en español.",
    max_tokens: 300,
    temperature: 0.7
  };
...
async function llamarMistral(prompt) {
  ...
  const body = {
    model: 'mistral-small',
    messages: [{ role: 'user', content: prompt + " Por favor responde en español." }],
    temperature: 0.7,
    max_tokens: 300
  };
